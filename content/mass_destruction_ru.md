...
* Доделал большую часть механики для гранатомёта, он теперь работает. Несколько изменил требования к MassDestruction 1.0: тип оружия гранаты заменил на гранатомёт потому, что у роботов уже есть гранатомёт и можно повторно использовать код. Требуется сделать звук для завершения перезарядки и для выпускания ракеты.
